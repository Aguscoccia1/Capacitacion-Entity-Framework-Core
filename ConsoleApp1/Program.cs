...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        using (var context = new AppDbContext())
        {
            ...
            await CompletarTarea(context, tareaIdACompletar);
            ...
            {
                Console.WriteLine();
                Console.WriteLine($"Proyecto: {proyecto.Nombre} - {proyecto.Id}");
            ...
            await EliminarProyecto(context, idProyectoAEliminar);
    ...
    static async Task CompletarTarea(AppDbContext context, int tareaId)
    {
        var tareaSeleccionada = await context.Tareas.FindAsync(tareaId);
        ...
        if (tareaSeleccionada != null)
        {
            ...
            Console.WriteLine($"Tarea '{tareaSeleccionada.Titulo}' completada correctamente.");
        }
        else
        {
            Console.WriteLine($"No se encontró ninguna tarea con el ID {tareaId}, por favor ingrese un ID válido");
        }
    }

    static async Task EliminarProyecto(AppDbContext context, int proyectoId)
    {
        var proyectoParaBorrar = await context.Proyectos
